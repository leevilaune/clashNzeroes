...
public class PlayerDao {
    ...
    public PlayerEntity findLatestByTag(String tag) {
        ...
        try {
            return em.createQuery(
                            "SELECT p FROM PlayerEntity p WHERE p.tag = :tag ORDER BY p.ts DESC", PlayerEntity.class)
    ...
    public List<Integer> findTrophies(String tag) {
        ...
        TypedQuery<Integer> query = em.createQuery(
                "SELECT p.trophies FROM PlayerEntity p WHERE p.tag = :tag ORDER BY p.ts ASC", Integer.class);
        query.setParameter("tag", tag);
